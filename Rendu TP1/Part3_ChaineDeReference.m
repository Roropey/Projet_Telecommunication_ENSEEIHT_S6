...
for k=1:length(E_bN0dB_2_oeil)
    [~, ~ , ~, ~, ~, z] = transmission(Fe,Rb,N,a,nb_bits,E_bN0dB_2_oeil(k),n0,h,0,hr);
    oeil_bruit = reshape(z, 2*(Fe/Rb), length(z)/(2*(Fe/Rb)));
    subplot(2,2,k);
    plot(oeil_bruit(:,1:100*(Fe/Rb)));
    title(strcat("Diagramme de l'oeil pour E_b/N_0", num2str(E_bN0dB_2_oeil(k)), "dB"));
    xlabel("Echantillons");
    ylabel("Amplitude");
end;
